...
// ── Claude AI (Anthropic) ────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are a warm, knowledgeable and friendly assistant for Mothibisi Senior Secondary School in Matatiele, Eastern Cape, South Africa.
...
async function askClaude(userMessage: string): Promise<string> {
  try {
    ...
    return 'I\'m having trouble connecting right now. Please contact the school directly at +27 76 707 3212 or office@mothibisisss.co.za.';
  }
}
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you! Whether it's admissions, boarding, fees, results, activities or anything else about Mothibisi SSS — just ask and I'll be happy to assist.",
    },
  ]);
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#800020] text-white shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-sm leading-tight truncate">Mothibisi Assistant</div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((m) => (
              ...
              >
                {m.role === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-[#800020] flex items-center justify-center shrink-0 mb-0.5">
            ...
            {isTyping && (
              <div className="flex items-end gap-1.5">
                <div className="w-6 h-6 rounded-full bg-[#800020] flex items-center justify-center shrink-0">
                  <Sparkles size={11} className="text-white" />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#FFFFFF]/25 text-[#FFFFFF] hover:bg-[#B89200] hover:text-white transition-colors font-medium shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#800020]/20 focus:border-[#FFFFFF]/40 transition-all bg-gray-50 placeholder:text-gray-400"
              ...
              <button
                ...
                className="bg-[#800020] hover:bg-[#B89200]/90 text-white px-3 py-2 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0"
