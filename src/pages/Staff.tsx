...
const StaffCard = ({ member }: { member: StaffMember }) => (
  <div
    ...
    style={ { background: '#FFFBEF', border: '1px solid #800020' } }
  >
    {/* Avatar */}
    <div
      className="w-24 h-24 rounded-full flex items-center justify-center mb-4 overflow-hidden"
      style={ { background: '#FDF9EC', border: '3px solid #800020' } }
    >
      {member.image ? (
        <img
          src={member.image}
        ...
        <User size={40} style={ { color: '#800020', opacity: 0.5 } } />
      )}
    </div>

    <h3 className="text-sm font-bold leading-tight" style={ { color: '#FFFFFF' } }>
      {member.name}
    </h3>
    <p className="text-xs font-semibold mt-1" style={ { color: '#800020' } }>
      {member.position}
    </p>
    {member.subject && (
      <span
        className="mt-2 inline-block text-xs font-medium px-3 py-1 rounded-full"
        style={ { background: '#FDF9EC', color: '#FFFFFF', border: '1px solid #800020' } }
      >
        {member.subject}
      </span>
    )}
    ...
    <div className="min-h-screen py-12 px-4" style={ { background: '#FDF9EC' } }>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10">
          <h1 className="text-4xl font-extrabold tracking-tight mb-3" style={ { color: '#FFFFFF' } }>
            Our Staff
          </h1>
          <div className="w-16 h-1 mx-auto rounded-full mb-4" style={ { background: '#800020' } } />
          <p className="text-gray-500 text-base max-w-2xl mx-auto">
            Meet the dedicated team of educators and support staff at Mothibisi Senior Secondary School.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-10">
          {categories.map(cat => (
            <button
              ...
              style={
                activeCategory === cat
                  ? { background: '#800020', color: '#FFFFFF', border: '2px solid #800020', fontWeight: 700 }
                  : { background: '#FFFBEF', color: '#FFFFFF', border: '2px solid #800020' }
              }
